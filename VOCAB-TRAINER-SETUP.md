...
4. **NOTION_ASSIGNMENTS_DB_ID** *(optional)* — logs every published set as a row in your "Assignments" database (inside the "Assignment Tracking" page), matching your existing tracker: Assignment Title, Format (Individual), Platform (practice), Skill (vocabulary), Type (vocpractice), Level (from the set), Time required (auto: ≤10 words → short, 11-20 → normal, >20 → huge), URL, and created date. To set it up: in Notion, open the "Assignments" database → "..." menu → Connections → add your integration. The database ID is `1bc8c843-86cd-8096-b09a-ef1318819d63`. Reuses your existing `NOTION_TOKEN`. If this isn't set, publishing still works — sets just aren't logged in Notion.
